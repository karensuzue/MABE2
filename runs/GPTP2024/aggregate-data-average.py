...
def main():
    ...
    # Make sure all average CSVs across all sets are moved to this folder
    avg_dir = os.path.join(data_dir, "average")
    if not os.path.exists(avg_dir):
        print("Average folder does not exist")
        return
    
    # For average files, group control and aged files with same conditions
    ...
    for entry in os.listdir(avg_dir):
        ...
        conditions_str = "_".join(conditions) # e.g Just explore_200_500_50
    ...
    # e.g. {"explore_200_500_50": [path_to_control.csv, path_to_diagnostic.csv]}
    for cond, files in avg_condition_groups.items():
        aggregate_file = os.path.join(avg_dir, f"{cond}.csv") # explore_200_500_50.csv

        for file in files: # [path_to_control.csv, path_to_diagnostic.csv]
            experiment_type = file.split('_')[1]
            file_data = pd.read_csv(file)
            file_data["Type"] = [experiment_type for i in range(len(file_data))]
