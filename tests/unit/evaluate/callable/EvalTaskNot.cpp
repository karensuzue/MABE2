 ...
 *  @date 2019-2021.
 *
 *  @file EvalTaskNot.cpp 
 *  @brief Test file for the NOT boolean logic task that is called via IO instruction
 */
...
TEST_CASE("EvalTaskNot", "[evaluate/callable]"){
  ...
  mabe::EvalTaskNot task(control);

  // Test evaluation on easy numbers
  CHECK(task.CheckOneArg(~0, 0));
  CHECK(task.CheckOneArg(~2, 2));
  CHECK(task.CheckOneArg(~3, 3));
  CHECK(task.CheckOneArg(~5, 5));
  ...
  task.AddOwnedTrait<bool>("not_performed", "Was and performed?", false);
  ...
  CHECK(!org.GetTrait<bool>("not_performed"));
  // Correct answer -> reward
  output_vec.push_back(~35); 
  action.function_vec[0].Call<void, org_t&, const inst_t&>(org, inst);
  CHECK(org.GetTrait<double>("merit") == 1);
  CHECK(org.GetTrait<bool>("not_performed"));
  // Repeated answer -> no reward
  output_vec.push_back(~12);
  action.function_vec[0].Call<void, org_t&, const inst_t&>(org, inst);
  CHECK(org.GetTrait<double>("merit") == 1);
  CHECK(org.GetTrait<bool>("not_performed"));
}
