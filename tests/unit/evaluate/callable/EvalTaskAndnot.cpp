 ...
 *  @date 2019-2021.
 *
 *  @file EvalTaskAndnot.cpp 
 *  @brief Test file for the ANDNOT boolean logic task that is called via IO instruction
 */
...
TEST_CASE("EvalTaskAndnot", "[evaluate/callable]"){
  ...
  mabe::EvalTaskAndnot task(control);

  // Test evaluation on easy numbers
  CHECK(task.CheckTwoArg(0, 0, 1));
  CHECK(task.CheckTwoArg(1, 1, 0));
  CHECK(task.CheckTwoArg(2, 2, 1));
  ...
  CHECK(task.CheckTwoArg(4, 5, 3));
  CHECK(task.CheckTwoArg(2, 3, 5));
  ...
  task.AddOwnedTrait<bool>("andnot_performed", "Was and performed?", false);
  ...
  CHECK(!org.GetTrait<bool>("andnot_performed"));
